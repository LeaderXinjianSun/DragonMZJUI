...
namespace DXH.PLC
{
    public class Delta_ModbusASCII
    {
        bool mClose = false;
        ...
        public Delta_ModbusASCII(String PortName, int BaudRate, Parity Parity, int DataBits, StopBits StopBits)
        {
        ...
        public bool PLCWriteStr(string mDevIndex, string mDevAdd, string mDevData)
        {
            ...
            lock (SerialLock)
            {
                ...
                {
                    Debug.Print("PLCWriteStr:" + ex.Message);
                    return false;
                }
            }
            if (mPrint)
        ...
        public async void PLCWriteAsync(string mDevIndex, string mDevAdd, string mDevData)
        {
            Task taskFunc = Task.Run(() =>
            {
                try
                {
                    PLCWrite(mDevIndex, mDevAdd, mDevData);
                    State = true;
                }
                catch
                {
